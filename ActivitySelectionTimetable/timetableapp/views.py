...
def ClassView(request):
    ...
    if request.method == 'POST':
        ...
        lst=request.POST.getlist("week_day")
        starting=request.POST.getlist("start_time")
        ending=request.POST.getlist("end_time")
        print(starting)
        time = [0] * (int(ending[0]) - int(starting[0]))
        time_slot = [''] * (int(ending[0]) - int(starting[0]))
        for x in range(0, len(time)):
                time_slot[x] = str(int(starting[0]) + x) + ':00 - ' + str(int(ending[0]) + x ) + ':00'
                time[x] = int(starting[0]) + x
        weak.append(lst[0])
        sloting.append(time_slot[0])
        if section.is_valid():  
...
def TimeTableView(request, id):
    try:
        ...
        week_days = Class.objects.values_list('week_day')
        professor_name=Professor.objects.values_list('professor_name').last()
        course_id=Course.objects.values_list('course_id').last()
        room_id=Classroom.objects.values_list('classroom_id').last()
        ...
        #weaking.append(weak[-1])
        #slot.append(sloting[-1])
        print(weak)
        print(sloting)

        #context_1 = {'section': section, 'courses': courses, 
        #             'professors':professors, 'rooms': rooms, 
        #             'activities': activities, 'time': time, 
        #             'w': time_slot  }
  
        context_1 ={'week_day': weak[0],
                    'time': sloting[0],
                    'professor_name':professor_name[0],
                    'course_id':course_id[0],
                    'room_id':room_id[0],
                    }
        return render(request, 'timetableapp/NewTimetable.html', context_1)
    except Class.DoesNotExist:
